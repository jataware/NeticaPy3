...
def SetNodeProbs(self, NewNode node_obj, *arg):
    cdef node_bn* node
    cdef node_bn* curNode
    cdef state_bn* parent_states = NULL
    cdef prob_bn* probs = NULL
    cdef char* statename
    ...
    if numparents > 0:
        ...
        if not parent_states:
            raise MemoryError("Failed to allocate memory for parent_states")

    if numstates > 0:
        probs = <prob_bn *>malloc(numstates * sizeof(prob_bn))
        if not probs:
            # free(parent_states)
            raise MemoryError("Failed to allocate memory for probs")
