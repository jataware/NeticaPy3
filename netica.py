...
class NeticaGraph:
    ...
    def set_node_probs(self, node: int | str | NeticaNode, probs: List[float]):
        """Set the probabilities for the states of a discrete node."""
        node = self.get_node(node)
        num_states = self.get_num_node_states(node)

        # Check if the number of probabilities matches the number of states
        if len(probs) != num_states:
            raise ValueError(f"Number of probabilities ({len(probs)}) must match the number of states ({num_states})")

        # Convert probabilities to the required format
        probs_bn = [State(prob) for prob in probs]

        # Call the SetNodeProbs_bn function from the Cython file
        N.SetNodeProbs_bn(node, probs_bn)
